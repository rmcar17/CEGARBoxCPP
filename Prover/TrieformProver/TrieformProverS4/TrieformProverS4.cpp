#include "TrieformProverS4.h"

Cache TrieformProverS4::persistentCache = Cache("P");

unsigned int TrieformProverS4::assumptionsSize = 0;
GlobalSolutionMemo TrieformProverS4::globalMemo = GlobalSolutionMemo();
unordered_map<string, unsigned int> TrieformProverS4::idMap =
    unordered_map<string, unsigned int>();

shared_ptr<Trieform>
TrieformFactory::makeTrieS4(const shared_ptr<Formula> &formula,
                            shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie = shared_ptr<Trieform>(new TrieformProverS4());
...
shared_ptr<Trieform>
TrieformFactory::makeTrieS4(const vector<int> &newModality,
                            shared_ptr<Trieform> trieParent) {
  shared_ptr<Trieform> trie = shared_ptr<Trieform>(new TrieformProverS4());
...
TrieformProverS4::TrieformProverS4() {}
TrieformProverS4::~TrieformProverS4() {}

shared_ptr<Trieform>
TrieformProverS4::create(const shared_ptr<Formula> &formula) {
  return TrieformFactory::makeTrieS4(formula, shared_from_this());
}
shared_ptr<Trieform>
TrieformProverS4::create(const shared_ptr<Formula> &formula,
                         const vector<int> &newModality) {
  return TrieformFactory::makeTrieS4(formula, newModality, shared_from_this());
}
shared_ptr<Trieform> TrieformProverS4::create(const vector<int> &newModality) {
  return TrieformFactory::makeTrieS4(newModality, shared_from_this());
}

shared_ptr<Bitset>
TrieformProverS4::convertAssumptionsToBitset(literal_set literals) {
...
void TrieformProverS4::updateSolutionMemo(const shared_ptr<Bitset> &assumptions,
                                          Solution solution) {
...
bool TrieformProverS4::isInHistory(vector<shared_ptr<Bitset>> history,
                                   shared_ptr<Bitset> bitset) {
...
void TrieformProverS4::reflexiveHandleBoxClauses() {
  ...
  for (auto modalSubtrie : subtrieMap) {
    dynamic_cast<TrieformProverS4 *>(modalSubtrie.second.get())
        ->reflexiveHandleBoxClauses();
  }
}

void TrieformProverS4::reflexivepropagateLevels() {
  for (auto modalSubtrie : subtrieMap) {
    dynamic_cast<TrieformProverS4 *>(modalSubtrie.second.get())
...
void TrieformProverS4::pruneTrie() {
  for (auto modalSubtrie : subtrieMap) {
    if (modalSubtrie.second->hasSubtrie(modalSubtrie.first)) {
      modalSubtrie.second->removeSubtrie(modalSubtrie.first);
    }
    dynamic_cast<TrieformProverS4 *>(modalSubtrie.second.get())->pruneTrie();
  }
}

void TrieformProverS4::makePersistence() {
  for (auto modalSubtrie : subtrieMap) {
    dynamic_cast<TrieformProverS4 *>(modalSubtrie.second.get())
        ->makePersistence();
  }
...
void TrieformProverS4::preprocess() {
...
void TrieformProverS4::prepareSAT(name_set extra) {
  // Shortcut only do this for level 1 as reflexivity guarantees every
  // possible assumption is here. Renaming could stuff this up
...
Solution TrieformProverS4::prove(vector<shared_ptr<Bitset>> history,
                                 literal_set assumptions) {
  // cout << "SOLVING LEVEL " << modality.size() << " HISTORY " <<
  // history.size()
  //      << endl;
  // Check solution memo
  ...
  for (auto modalityDiamonds : triggeredDiamonds) {
    ...
    while (!diamondPriority.empty()) {
      ...
      if (getLevel() == 1) {
        ...
        childSolution = dynamic_cast<TrieformProverS4 *>(
...
Solution TrieformProverS4::prove(literal_set assumptions = literal_set()) {
